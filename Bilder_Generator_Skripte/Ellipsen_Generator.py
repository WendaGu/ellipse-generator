...
def ellipse_generator(nx, ny, num_min, num_max, ab_min, ab_max, b_min, b_max,
    ...
    fig, ax = plt.subplots(figsize=(30, 30), dpi=10, subplot_kw={'aspect': 'equal'})
    used_point_list = []
    ...
    while k < counter:
        ...
        ell = matplotlib.patches.Ellipse(xy=(np.random.uniform(0, nx), np.random.uniform(0, ny)),
                                         width=b, height=a, angle= choice(winkel) * 90, alpha=None,
            ...
            if len(intersection) == 0:
                ...
                if abschneiden is False:
                    if np.ceil(bb.x0) > 2 and np.floor(bb.x1) < nx - 2 and np.ceil(bb.y0) > 2 and np.floor(
                        ...
                        x2 = int(x_high + (ell.height * np.sin(ell_angle) / 2))
                        y2 = int((y_high - (ell.height * np.cos(ell_angle) / 2)))
                        x3 = int(2 * x_center - x1)
                        y3 = int(2 * y_center - y1)
                        x4 = int(2 * x_center - x2)
                        y4 = int(2 * y_center - y2)
                        x1_list.append(x1)
                        y1_list.append(300-y1)
                        x2_list.append(x2)
                        y2_list.append(300-y2)
                        x3_list.append(x3)
                        y3_list.append(300-y3)
                        x4_list.append(x4)
                        y4_list.append(300-y4)
    ...
    dir2 = 'C:/Users/Wenda Gu/Desktop/Test/Horizontal/Images/'
    # dir2 = 'C:/Users/Wenda Gu/Desktop/RRCNN/VOC/VOCdevkit/test/JPEGImages/'
    filename2 = dir2 + num + '.jpeg'
    ...
    # print(x_max)
    # print(y_max)
    dom = xml.dom.minidom.parse("C:/Users/Wenda Gu/Desktop/RRCNN/VOC/VOCdevkit/VOC2007/test_20.xml")
    ...
    x4 = root.getElementsByTagName('x4')
    y4 = root.getElementsByTagName('y4')
    ...
    for n in range(num_min):
        ...
        x4[n].firstChild.data = x4_list[n]
        y4[n].firstChild.data = y4_list[n]
    filename3 = "C:/Users/Wenda Gu/Desktop/Test/Horizontal/Annotation/" + num + '.xml'
